...
public:
    //MppEncodeData();
    MppEncodeData(int fd, int width, int height, uint8_t *pInputData);
    ~MppEncodeData();
    void startup();
    void init_mpp();
...
private:
    void rtp_transmit(void *ptr, size_t len);
    bool process_image();
};

void MppEncodeData::startup()
{
    process_image();
}

void MppEncodeData::rtp_transmit(void *ptr, size_t len)
{
    struct rtp_data pdata;
    unsigned char *rtpdata = (unsigned char*)malloc(len);
    if(NULL == rtpdata)
    {
        perror("MppEncodeData::rtp_send");
        return;
    }
    memcpy(rtpdata, ptr, len);
    pdata.buff = rtpdata;
    pdata.bufrelen=0;
    /* 数据封包 */
...
bool MppEncodeData::process_image()
{
    MPP_RET ret = MPP_OK;
    MppFrame frame = NULL;
    ...
    {
        printf("mpp encode put frame successed\n");
    }
...
void MppEncodeData::init_mpp()
{
        ...
        mpp_enc_data.fps = 20;
        mpp_enc_data.gop = 60;
        //mpp_enc_data.bps = 3 * mpp_enc_data.width * mpp_enc_data.height / 8 *mpp_enc_data.fps;
...
MppEncodeData::MppEncodeData(int fd, int width, int height, uint8_t *pInputData)
{
    this->height = height;
    this->width = width;
    this->fd = fd;
