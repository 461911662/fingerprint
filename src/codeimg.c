...
#define UDP_MAX_PACK_SIZE 65535
void encoderImg(X264Encoder &x264Encoder,AVFrame *frame)
{
	//转换图像格式
	...
	for (int i = 0; i < x264Encoder.m_x264iNal; ++i)
	{
        ...
        if(0 == i)
        {
            pdata->datalen=x264Encoder.m_pX264Nals[i].i_payload + 5; /* start */
            
            unsigned char *rtpdata = (unsigned char *)malloc(pdata->datalen);
            sprintf(rtpdata, "%s", "start");
            memcpy(rtpdata+5, x264Encoder.m_pX264Nals[i].p_payload, pdata->datalen);
            pdata->buff=rtpdata;
            pdata->bufrelen=0;
            printf("buff len : %d\r\n", pdata->datalen);
        }
		...
		if(pdata->datalen > UDP_MAX_PACK_SIZE)
		{
			free(pdata);
            //rtpdata = NULL;
            pdata=NULL;
